...
#          outputFile.write("{:21} {:10} {:7} {:8} {:8} {:8}".format(p.getName(), str(p.getWins()) + "-" + str(p.getLosses()), p.getSpread(), p.getInitRating(), p.getNewRating(), p.getNewRatingDeviation()))
  ...
  def outputRatfile(self, outFile):
    ...
    for p in sorted(self.globalList.getAllPlayers().values(), key=lambda p: (p.getNewRating()), reverse=True):
      if p.getName() in ["Yy bye", "A Bye", "B Bye", "ZZ Bye", "Zz Bye", "Zy bye", "Bye One", "Bye Two", "Bye Three", "Bye Four", "Y Bye", "Z Bye"]:
        continue
      try:
  ...
  def outputActiveRatfile(self, outFile):
    ...
    try:
      for p in sorted(self.globalList.getAllPlayers().values(), key=lambda p: (p.getNewRating()), reverse=True):
        active = (p.getLastPlayed() > self.tournamentDate - datetime.timedelta(days=731))
...
class Player(object):
  ...
  def adjustInitialDeviation(self, tournamentDate):
    try:
      ...
      print "DEBUG {0} {1} {2} {3}".format(self.name, self.lastPlayed, inactiveDays, self.initRatingDeviation)
      exc_type, exc_obj, exc_tb = sys.exc_info()
      template = "An exception of type {0} occured at line {1}. Arguments:\n{2!r}"
      message = template.format(type(ex).__name__, exc_tb.tb_lineno, ex.args)
      print message
...
#        tau = 90
      ...
      sigma = float(self.initRatingDeviation) # Deviation is adjusted for inactive time when player is loaded
...
#      print "{0} {1}".format(self.name, delta)
      ...
      if self.careerGames > 1000:
        ...
        multiplier = min(multiplier, 1.0 - (float(self.careerGames)/1800))

      muPrime = mu + (delta*multiplier)

    except Exception as ex: 
        print "Error calculating ratings for {0}".format(self.name)
        print "beta {0} tau {1} mu {2} sigma {3}".format(beta, tau, mu, sigma)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        template = "An exception of type {0} occured at line {1}. Arguments:\n{2!r}"
        message = template.format(type(ex).__name__, exc_tb.tb_lineno, ex.args)
        print message
